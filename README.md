...
- 新采集数据的align结果
  ...
  <img src="image/0131-1.png" alt="0131-1" style="zoom:50%;" />
...
- 纯Apriltag恢复pose的结果
  ...
  ![0201-2](image/0201-2.png)

- 纯Colmap进行Image位姿估计的个结果

  ![0201-3](image/0201-3.png)

  其中colmap见图使用外部穿入的参数

  ![0201-4](image/0201-4.png)

- 发现之前用evo_traj话轨迹的时候正好差了一个逆,存在bug,已修复

  ![0201-5](image/0201-5.png)
...
- 使用colmap进行的稠密重建
  ...
  ![0202-1](image/0202-1.png)

- 优化结果

  ![0202-2](image/0202-2.png)

  红色为优化后,黄色为优化前

- 下一步,将相机参数从五个自由pose修改为 1 个自由pose + 4个相对pose

  ![0202-3](image/0202-3.png)
  ...
  ```
   cam1 pose 0 = -0.0485857 0.135723 -0.244541 -0.321114 -0.0147088 0.946694 0.0209748
   cam1 pose 1 = 0.170582 0.140898 -0.452368 0.66587 0.0126056 -0.745672 -0.0207909
   cam1 pose 2 = -0.0703004 0.123238 0.423734 -0.251402 -0.0167631 0.967491 0.0218601
   cam1 pose 3 = 0.923686 0.131545 0.303027 0.559366 0.0155254 -0.828466 -0.0226515
   cam1 pose 4 = -0.514409 0.115909 0.750253 0.0494209 -0.0155203 0.998479 0.0188959
   cam1 to cam 1 pose  = -6.93889e-18 2.77556e-17 0 1 -3.46945e-18 4.16334e-17 0
   cam1 to cam 2 pose  = 0.156436 -0.000868648 -0.119435 0.805836 -0.00232152 0.592134 -0.00039844
   cam1 to cam 3 pose  = 0.0946539 -0.000862442 -0.304277 0.308983 -0.0097798 0.951017 -0.000313702
   cam1 to cam 4 pose  = -0.097055 0.00276807 -0.303732 -0.296674 0.00259925 0.954955 0.00628071
   cam1 to cam 5 pose  = -0.159647 0.00132047 -0.116511 0.80402 -0.00537274 -0.594577 -0.00136271
  ```
...
- 激光 - Cam1 外参标定
  ...
  ![0204-3](image/0204-3.png)

  使用ICP将视觉获得的AprilTag与激光采集的点云进行ICP,获得Cam1与激光之间的外参变换

- 点云着色与遮挡

  ![0204-1](image/0204-1.png)

  ![0204-2](image/0204-2.png)

  在获得外参后,获得最终的点云着色结果如上二图所示

#### 2021-02-05

- 单帧Mesh生成与纹理贴图

  - 借助Meshlab的泊松重建恢复场景mesh,借助mvs-texturing对mesh进行场景纹理贴图

  ![0205-1](image/0205-1.png)

  ![0205-2](image/0205-2.png)
...
- 多帧融合与点云生成
  ...
  - 由于单帧视角有限,进行多帧的融合与贴图
    ...
    ![0205-3](image/0205-3.png)

    - 多帧重建结果

    ![0205-4](image/0205-4.png)